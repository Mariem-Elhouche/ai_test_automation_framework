...
public class EnvironmentDocumentPage extends BasePage implements DocumentListPage {

    private static final Logger LOGGER = Logger.getLogger(EnvironmentDocumentPage.class.getName());
    ...
    private static final String SEC_GROUPES = "//span[contains(.,'envrionnement')]/ancestor::div[contains(@class,'col-10')][1]/following-sibling::div[1]";
    private static final String SEC_ENTREPRISES = "//span[contains(.,'entreprises de cet environnement')]/ancestor::div[contains(@class,'col-10')][1]/following-sibling::div[1]";
    private static final String SEC_CATEGORIES = "//span[contains(.,'categories de cet environnement')]/ancestor::div[contains(@class,'col-10')][1]/following-sibling::div[1]";
    private static final String SEC_RH = "//h2[contains(.,'espace RH')]/following-sibling::div[1]";
    ...
    private WebElement getSectionContent(String sectionName) {
        ...
        if ("categories".equals(normalized) || "cat\u00e9gories".equals(normalized)) {
            List<WebElement> candidates = driver.findElements(By.xpath(
                    "//span[contains(.,'categories de cet environnement')]/ancestor::div[contains(@class,'col-10')][1]/following-sibling::div[position()<=2]"));
            if (!candidates.isEmpty()) {
                for (WebElement candidate : candidates) {
                    boolean hasCards = !candidate.findElements(By.xpath(CARD)).isEmpty();
                    boolean hasLoad = !candidate.findElements(By.xpath(LOAD_BTN)).isEmpty();
                    boolean hasNoDoc = !candidate.findElements(By.xpath(NO_DOC_MSG)).isEmpty();
                    if (hasCards || hasLoad || hasNoDoc) {
                        return candidate;
                    }
                }
    ...
    public void clickShowCategories() {
        getCardsInSection("cat\u00e9gories");
    }

    @Override
    public void clickShowRhDocuments() {
        getCardsInSection("rh");
    }
